...
@Component({
  selector: 'app-chart-campus',
  templateUrl: './chart-campus.component.html',
  styleUrls: ['./chart-campus.component.scss']
})
export class ChartCampusComponent implements OnInit {
  ...
  getChart(std:AnsStd[]){
    ...
    this.dataSeries.push((std.filter(
      user => user.campus_id === '1').length)/ per * 100,(std.filter(
        user => user.campus_id === '2').length)/ per * 100,(std.filter(
          user => user.campus_id === '3').length)/ per * 100,(std.filter(
            user => user.campus_id === '4').length)/ per * 100,(std.filter(
              user => user.campus_id === '5').length)/ per * 100)
              ...
              title: {
                text: 'สัดส่วนของนักศึกษาที่ตอบแบบสอบถาม'
              },
              legend: {
                position: 'right',
